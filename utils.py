...
def calcularCapacidadEspecial(matriz_canal):
    ...
    if esCanalUniforme(matriz_canal):
        ...
        entropia_fila = calculoEntropia(matriz_canal[0])
...
def estimarCapacidadCanalBinario(matriz_canal, paso = 0.0001):
    ...
    for i in range(num_pasos + 1):
        ...
        info_mutua_actual = calcularInformacionMutua(probs_a_priori, matriz_canal)
...
# Imprime un bytearray como lista decimal y como matriz binaria
def imprimirBytearray(byte_array):
    print(f"Decimal: {list(byte_array)}")
    print("Binario (fila por fila):")
    for byte_val in byte_array:
        bits_string = bin(byte_val)[2:].zfill(8)
        print(f"  {bits_string}")
...
if asd == 1:
    ...
    capacidad = calcularCapacidadEspecial(matriz_canal)
    print(f"Capacidad (C): {capacidad:.{decimales}f} bits")

    capacidad_estimada, probabilidad_optima = estimarCapacidadCanalBinario(matriz_canal)
    ...
    prob_error = calcularProbabilidadError(probs_priori, matriz_canal)
